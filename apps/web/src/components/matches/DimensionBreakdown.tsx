...
export function DimensionBreakdown({ dimensions }: DimensionBreakdownProps) {
  if (dimensions.length === 0) {
    return (
      <p className="text-sm text-zinc-400">No scoring breakdown available.</p>
    );
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          {isNullHandled && (
            <span
              className="text-xs text-zinc-400 border border-zinc-200 bg-zinc-100 rounded px-1.5 py-0.5"
        ...
        <span className="text-sm font-semibold text-spf-navy">
      ...
      <div className="h-2 bg-zinc-100 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-700 bg-gradient-to-r from-spf-navy to-blue-600 ${
      ...
      {/* Rationale */}
      {dim.rationale && (
        <p className="text-xs text-zinc-400 mt-1">{dim.rationale}</p>
      )}
    </div>
  );
}
