...
import { UploadCloud, Loader2, Check, X, Sparkles, Camera } from "lucide-react";
...
export function ResumeUploader({
  token, onApplied,
}: { token: string; onApplied?: (fields: Record<string, unknown>) => void }) {
  ...
  if (status === "done") {
    return (
      <div className="rounded-xl border border-cohere-green/30 bg-wash-green p-5 text-body text-cohere-ink">
        <Check className="inline h-4 w-4 mr-1.5 text-cohere-green" />
        Done — the fields below are filled in. Edit anything that doesn't look right.
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-hairline bg-white p-6 shadow-[0_1px_2px_rgba(12,10,9,0.04)]">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-display text-feature text-cohere-ink">Here's what we read</h3>
      </div>
      ...
      {(parsed.certifications.length > 0 || parsed.skills.length > 0) && (
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {parsed.certifications.length > 0 && (
            <div>
              <MonoLabel className="mb-1.5 block">Certifications found</MonoLabel>
              <div className="flex flex-wrap gap-1.5">
                {parsed.certifications.map((c) => (
                  <span key={c} className="rounded-full border border-cohere-green/30 bg-wash-green px-2.5 py-0.5 text-micro text-cohere-ink">{c}</span>
                ))}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {parsed.skills.slice(0, 12).map((s) => (
                  <span key={s} className="rounded-full border border-hairline bg-stone/40 px-2.5 py-0.5 text-micro text-slate">{s}</span>
                ))}
              </div>
            </div>
          )}
      ...
      <div className="mt-5 flex items-center justify-end gap-2">
        ...
        <button onClick={apply} disabled={status === "applying" || shownFields.length === 0} className="btn-primary inline-flex items-center gap-1.5">
