...
export function ChatClient({
  ...
  return (
    <div className="flex flex-col bg-white border border-neutral-200 rounded-lg overflow-hidden" style={{ minHeight: "500px" }}>
      {/* Messages area */}
      ...
      <div className="border-t border-neutral-100 p-3">
        ...
        {!isActive && (
          <p className="text-xs text-neutral-500 mb-2">This session is closed.</p>
        )}
        <div className="flex gap-2 items-end">
          <textarea
            ...
            className="flex-1 border border-neutral-200 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-1 focus:ring-neutral-400 disabled:bg-neutral-50"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || sending || !isActive}
            className="shrink-0 p-2 bg-neutral-900 text-white rounded-lg hover:bg-neutral-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            {sending ? (
