  ...
  return (
    <main className="p-6 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          <BackLink />
          <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 mt-1">Analytics</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {stats.map(({ label, value, icon: Icon }) => (
            <div
              key={label}
              className="bg-zinc-50 border border-zinc-200 rounded-lg p-4 text-center"
            >
              <Icon className="w-6 h-6 mx-auto mb-2 text-zinc-400" />
              <div className="text-3xl font-bold leading-none text-spf-navy">{value}</div>
        ...
        {/* Recent outreach */}
        <section>
          <h2 className="font-semibold text-zinc-900 mb-3">Recent outreach</h2>
          {data.recent_outreach.length === 0 ? (
            <div className="bg-zinc-50 border border-zinc-200 rounded-lg p-8 text-center">
              <p className="text-zinc-400 text-sm">No outreach sent yet.</p>
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-zinc-500">{o.job_title}</p>
                      {o.subject && (
